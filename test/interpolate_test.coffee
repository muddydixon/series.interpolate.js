...
describe 'Interpolate', ->
  ...
  describe 'interpolate Date array', ->
    it 'with no opt', ->
      correct = [0..testDataNum].map (d)-> new Date(2013, 0, d).getTime()
      expect(interpolate([0, 3, 5, testDataNum].map((d)->
        new Date(2013, 0, d).getTime()))[0]).to.deep.equal correct
        
    it 'with no opt and object containing Date', ->
      correct = [0..testDataNum].map (d)-> {x: new Date(2013, 0, d).getTime()}
      expect(interpolate([0, 3, 5, testDataNum].map((d)->
        {x: new Date(2013, 0, d).getTime()}))[0]).to.deep.equal correct
      
  describe 'interpolate pseude time series', ->
    it 'with no opt', ->
      correct = [0..testDataNum].map (d)->
        {v: 100 - d, x: new Date(2013, 0, d).getTime()}
