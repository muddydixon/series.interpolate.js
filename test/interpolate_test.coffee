...
describe 'Interpolate', ->
  describe 'interpolate simple number array', ->
    it 'with no opt', ->
      ...
      expect(interpolate([0, 1, 3, 5, 6, testDataNum])).to.deep.equal(correct)

    it 'with stepwise', ->
      correct = [0..testDataNum * 2].map (d)-> d / 2
      interpolate([0, 0.5, 3, 5.5, 6, testDataNum], {step: 0.5}).forEach (d, idx)->
        expect(d).to.deep.equal correct[idx]

    it 'with no opt 3 step', ->
      correct = d3.range(0, 30, 3)
      expect(interpolate([0, 3, 9, 27])).to.deep.equal(correct)
      
    it 'with no opt 3 step complamenting const value', ->
      correct = [0, 3, 10, 9, 10, 10, 10, 10, 10, 27]
      expect(interpolate([0, 3, 9, 27], {defaults: 10})).to.be.eql correct
      
        
  describe 'interpolate object array', ->
    it 'with no opt', ->
      correct = [0..testDataNum].map (d)-> {x: d}
      interpolate([{x:0},{x:1}, {x:testDataNum}]).forEach (d, idx)->
        expect(d).to.deep.equal correct[idx]
        
    it 'with stepwise', ->
      correct = [0..testDataNum * 2].map (d)-> {x: 2 * d}
      interpolate([0,4,16,testDataNum].map((d)-> {x:d}), {step: 2}).forEach (d, idx)->
        expect(d).to.deep.equal correct[idx]

    it 'with no opt 4 step', ->
      correct = d3.range(0, 30, 4).map (d)-> {x: d}
      
      interpolate([0, 4, 16, 28].map((d)-> {x: d})).map (d, idx)->
        expect(d).to.deep.equal(correct[idx])
      
    it 'with no opt 3 step complamenting const value', ->
      correct = [0, 3, 10, 9, 10, 10, 10, 10, 10, 27].map (d, idx)-> {x: idx * 3, v: d * d}

      expect(interpolate([0, 3, 9, 27].map((d)-> {x: d, v: d * d}), {defaults: {v: 100}})).to.be.eql correct
